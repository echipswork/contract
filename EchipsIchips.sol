...
contract EChipsERC20 is IERC20 {
    using SafeMath for uint256;
    using Address for address;
...
interface EchipsPotato {
	function burn(uint256 amount) external;
}

contract EchipsIchips is EChipsERC20, Ownable, ReentrancyGuard {
	using SafeMath for uint256;
	...
	constructor (
		address _potatoes
	) public EChipsERC20("e-chips.work", "Ichips") Ownable(){
		token = EchipsPotato(_potatoes);
		potatoes = IERC20(_potatoes);
	}
	...
	function durationRemaining(address account) external view returns (uint256) {
		...
		if(userFarm[account].balances > 0){
			if(userFarm[account].periodFinish > now){
				timeRemaining = userFarm[account].periodFinish - now;
			}
		}
	...
	function harvest() external nonReentrant{
		...
		token.burn(userFarm[msg.sender].balances);
